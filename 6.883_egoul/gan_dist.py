...
labels_placeholder = tf.placeholder(tf.int32, shape = [1,10])
Gz = generator(z_placeholder, batch_size, z_dimensions)
# Gz holds the generated images

Dx = discriminator(x_placeholder, labels_placeholder)
# Dx will hold discriminator prediction probabilities
# for the real MNIST images

Dg = discriminator(Gz, labels_placeholder, reuse_variables=True)
...
for i in range(10):
    ...
    labels = mnist_classifier.summary_statistics(modified_batch)
    labels = np.reshape(labels, [1,10])
    _, __, dLossReal, dLossFake = sess.run([d_trainer_real, d_trainer_fake, d_loss_real, d_loss_fake],
                    {x_placeholder: real_image_batch, labels_placeholder: labels, z_placeholder: z_batch})
...
for i in range(10):
    ...
    # Train generator
    z_batch = np.random.normal(0, 1, size=[batch_size, z_dimensions])
    _ = sess.run(g_trainer, feed_dict={z_placeholder: z_batch, labels_placeholder: labels})

    if i % 10 == 0:
        # Update TensorBoard with summary statistics
        z_batch = np.random.normal(0, 1, size=[batch_size, z_dimensions])
        summary = sess.run(merged, {z_placeholder: z_batch, x_placeholder: real_image_batch, labels_placeholder: labels})
